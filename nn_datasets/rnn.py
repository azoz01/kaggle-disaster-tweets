    ...
    def __init__(self, text_type: str,
        ...
        Args:
            text_type (str): Type of text to be read. So far available:
                ...
                - lemmatized
                - stemmed
            min_word_freq (int): words with lower frequency in text than that
                are removed. Defaults to 3.
            min_sentence_len (int): sentences with lower length after word
                removal are dropped with corresponding entries in target.
                Defaults to 0.
        """
        df_text = pd.read_csv(os.path.join('prepared', f'{text_type}.csv'))
        y = pd.read_csv('prepared/target.csv')

        df_text_filered_tokenized = \
            tokenize_and_filter_irrevelant_words(df_text, min_word_freq)
        df_text_filered_tokenized, y = \
            filter_tokenized_by_sentence_len(df_text_filered_tokenized,
                                             y,
                                             min_sentence_len)
