...
def plot_learning_curve(estimator: object, X: np.ndarray, y: np.ndarray) -> plt.figure:
    """Returns figure with learning curve for train and validation set.
       Lines are mean accuracy, shaded area is mean +/- standard deviation of accuray and points are min/max accuracy
    ...
    plt.fill_between(sizes, train_mean - train_std, train_mean + train_std, alpha = .1)
    ...
    plt.fill_between(sizes, valid_mean - valid_std, valid_mean + valid_std, alpha = .1)
