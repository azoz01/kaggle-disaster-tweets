...
def encode_df_text(df_text: pd.DataFrame,
                   target: pd.DataFrame,
                   min_word_freq: int,
                   min_sentence_len: int,
                   embedding: Callable[[str], int]
                   ) -> tuple[np.ndarray, np.ndarray]:
    """
    Tokenizes sentences, removes words with lower than min_word_freq,
    removes entries from df_text and target when lenght of entry is df_text is
    lower than min_sentence_len and encodes using embedding mapping
    ...
    Args:
        ...
        embedding (Callable[[str], int]): mapping from string to vector
            with fixed length

    Returns:
        Tuple[np.ndarray, np.ndarray]: first are embedded sentences
            from df_text, second target (after filtering)
    """
    tokenized = tokenize_and_filter_irrevelant_words(df_text, min_word_freq)
    tokenized, target = filter_tokenized_by_sentence_len(tokenized,
                                                         target,
                                                         min_sentence_len)
