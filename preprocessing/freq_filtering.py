...
def tokenize_and_filter_irrevelant_words(df_text: pd.DataFrame,
                                         min_word_freq: int
                                         ) -> pd.DataFrame:
    """
    ...
    def frequency_filter(row):
        return [word for word in word_tokenize(row[0])
                if word in significant[:, 0]]

    df_text_filered_tokenized = df_text.apply(frequency_filter, axis=1)
    return df_text_filered_tokenized


def filter_tokenized_by_sentence_len(tokenized_df: pd.DataFrame,
                                     target: pd.DataFrame,
                                     min_sentence_len: int
                                     ) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Removes entries from datasets where length
    of sentence is lower than min_len

    Args:
        tokenized_df (pd.DataFrame): dataframe with 'text' column
            containing tokenized sentences
    ...
    """
    len_mask = np.array(list(map(len,
                                 tokenized_df.values
                                 ))) >= min_sentence_len
    return (tokenized_df.loc[len_mask].reset_index(drop=True),
            target.loc[len_mask].reset_index(drop=True))
