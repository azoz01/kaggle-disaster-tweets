...
from typing import Callable, Iterable, DefaultDict
...
def encode_df_tokenized(df_tokenized: pd.DataFrame,
                        embedding: Callable[[str], np.ndarray]
                        ) -> np.ndarray:
    """
    Args:
        df_tokenized (pd.DataFrame): dataframe containing column 'text'
            with lists of words
        embedding (Callable[[str], np.ndarray]): mapping from string
            to vector with fixed length
...
def __get_id_dict_of_words(df_tokenized: pd.DataFrame) -> dict[str, int]:
    """
    Creates dictionary which keays are words from df_tokenized
    and values are unique ids for each word
    ...
    Returns:
        dict[str, int]: dictionary where words are keys
            and values are unique ids
    """
...
def one_hot(words: list[str],
            id_dict: dict[str, int]
            ) -> np.ndarray:
    """
    Args:
        words (list[str]): list of words
        id_dict (dict[str, int]): dictionary containing words in keys
            and ids in values
...
def word_count_encoding(words: list[str],
                        id_dict: dict[str, int]
                        ) -> np.ndarray:
    """
    Args:
        words (list[str]): list of words
        id_dict (dict[str, int]): dictionary containing words
            in keys and ids in values

    Returns:
        np.ndarray: vector where i'th element contains number
            of occurences words with id i
    """
    ids = [id_dict[word] for word in words]
    counter = Counter(ids)
    vector = np.zeros((len(id_dict)))
